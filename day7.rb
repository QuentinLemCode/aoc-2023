...
def part2(input)
  ...
  hands.sort do |hand1, hand2|
    rank1, rank2 = hand_rank2(hand1[0]), hand_rank2(hand2[0])
    if rank1 == rank2
      cards1 = hand1[0].chars.map { |card| CARD_VALUES_2[card] }
      cards2 = hand2[0].chars.map { |card| CARD_VALUES_2[card] }
      sortval = 0
      cards1.zip(cards2).each do |card1, card2|
        next if card1 == card2
        sortval = card1 < card2 ? 1 : -1
...
def hand_rank2(hand)
  occ = hand.chars.map{ |x| CARD_VALUES_2[x] }.tally
  joks = occ.delete(1)
  return 7 if occ.empty?

  counts = occ.values.sort.reverse
  counts[0] += joks if joks
  case counts.reverse
  when [1, 4]
    6
  when [2, 3]
